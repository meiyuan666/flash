...
DATA_DIR = os.path.join('../data/', 'realworld_json')
FINAL_DIR = os.path.join('../data', 'realworld_leaf')
SEQ_LEN = 10
MAX_SAMPLE = 300
    ...
    with open(os.path.join(DATA_DIR, file_name)) as json_file:
        ...
        for u in data['users']:
            ...
            if cnt % 10000 == 0 and cnt > 0:
                save_json(train_json, 'train', file_name.replace('.json', '') + '_{}'.format(cnt/10000))
                save_json(test_json, 'test', file_name.replace('.json', '') + '_{}'.format(cnt/10000))
                train_json = create_leaf_json(data)
                test_json = create_leaf_json(data)
            cnt+=1
            ...
            train_json['users'].append(u)
            test_json['users'].append(u)
            # print(len(u_data['y']))
            num_samples = min(MAX_SAMPLE, len(u_data['y']))
            train_json['num_samples'].append(num_samples)
            test_json['num_samples'].append(num_samples)
            sample_x = np.random.choice(u_data['x'], num_samples, replace = False)
            sample_y = np.random.choice(u_data['y'], num_samples, replace = False)   # y is always [{},{},...,{}]
            ordered_data = {'x':sample_x, 'y':sample_y}
            
            train_thres = int(0.8 * num_samples)
